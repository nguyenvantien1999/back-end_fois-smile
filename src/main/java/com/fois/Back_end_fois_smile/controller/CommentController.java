...
public class CommentController {
    ...
    @RequestMapping(value = "/comment/addcomment", produces = "application/json")
    @ResponseBody
    ...
    @RequestMapping(value = "/comment/getcomment", produces = "application/json")
    @ResponseBody
    public ArrayList<CommentForm> getByMaBH(String mabh) {
        return this.commentMapper.selectByMabh(mabh);
    }
    
    @MessageMapping("/cmtSocket")
    @ResponseBody
    @SendTo("/topic/getCmtSocket")
    public ArrayList<CommentForm> getCmtSocket(String mabh) throws Exception {
        Thread.sleep(1000);
        return getByMaBH(mabh);
    }
}
